# World Crests and Feeling Lines

## Status

Provisional v1.1 world naming and feeling-line set.

Date recorded: 2026-06-27

This file defines the current working world names, temporary/provisional place names, World Crest names, and feeling lines for the 64-page ApexCrest first edition.

The place names in this file are **not final locks**. They are the current best temporary set, recorded so the repo stays aligned while world names are audited next. They may be reopened after the world-name audit, visual testing, and chapter prototyping.

## Purpose

Each world needs a line that makes the customer instantly understand what kind of card belongs there without using mechanical card-placement language.

The feeling line must create the reaction:

**I know exactly which card belongs here.**

It must also support the stronger reaction:

**A binder does not do this.**

## Core rule

The line may use the word **card** because the emotional history belongs to the card.

The page/system uses **Crest** because the Crest is the symbolic recognition moment.

Correct hierarchy:

- World: **Ember Tribute**
- Place: **Volcanic Valley**
- World Crest: **Ember Crest**
- Feeling line: **For the card that still glows after the moment has passed.**

Do not replace every use of **card** with **Crest**.

Do not use **card** with mechanical verbs such as insert, store, swap, organise, protect, catalogue, or mount in customer-facing copy.

## Current working world set

### 1. Ember Tribute

Place:

**Volcanic Valley**

World Crest:

**Ember Crest**

Feeling line:

**For the card that still glows after the moment has passed.**

Score:

**96/100**

Reason:

The full stack is strong: Ember Tribute, Volcanic Valley, Ember Crest, and the feeling line all reinforce heat, pressure, emotional residue, and collector attachment.

Status:

**Benchmark line. Do not dilute.**

---

### 2. Lagoon Celebration

Place:

**Shellwater Cove**

World Crest:

**Lagoon Crest**

Feeling line:

**For the card that feels like joy you wanted to keep.**

Score:

**89/100**

Reason:

Shellwater Cove is ownable, enterable, tactile, and more premium than Tropical Water Haven while keeping Lagoon’s joy, softness, and water-world identity.

Status:

**Temporary place-name lock. Recheck after world-name audit and visual testing.**

---

### 3. Forest Remembrance

Place:

**The Elder Root**

World Crest:

**Forest Crest**

Feeling line:

**For the card that brings an old memory back.**

Score:

**91/100**

Reason:

The Elder Root is stronger than Ancient Woodland Path because it carries memory, origin, roots, ancestry, and woodland atmosphere without sounding like generic fantasy geography.

Status:

**Temporary place-name lock. Recheck after world-name audit and visual testing.**

---

### 4. Moonlight Discovery

Place:

**The Moonlit Arches**

World Crest:

**Moonlight Crest**

Feeling line:

**For the card that reveals more the longer you look.**

Score:

**91/100**

Reason:

The Moonlit Arches is more ownable, more visual, and more enterable than Moonlit Ruins and Water. The feeling line gives Moonlight a clear role: mystery, hidden detail, and cards that reward attention.

Status:

**Temporary place-name lock. Recheck after world-name audit and visual testing.**

---

### 5. Prism Harmony

Place:

**The Refraction Pools**

World Crest:

**Prism Crest**

Feeling line:

**For the card that changes every time the light finds it.**

Score:

**93/100**

Reason:

The Refraction Pools is stronger than Valley of Light because it focuses on what light does rather than using light as a generic fantasy noun. It fits holo, shiny, rainbow, reflective, rare, and visually beautiful cards.

Status:

**Temporary place-name lock. Recheck after world-name audit and visual testing.**

---

### 6. Storm Ascent

Place:

**Storm Coast**

World Crest:

**Storm Crest**

Feeling line:

**For the card that came through the hard part.**

Score:

**90/100**

Reason:

Storm owns struggle, difficulty, conflict, and endurance. Storm Coast is cleaner than Lightning Pass because it avoids mountain overlap with Summit.

Status:

**Temporary place-name lock. Recheck after world-name audit and visual testing.**

---

### 7. Frost Resolve

Place:

**Frozen Lake**

World Crest:

**Frost Crest**

Feeling line:

**For the card that endured the wait.**

Score:

**91/100**

Reason:

This line gives Frost patience, restraint, time, and quiet endurance. It is clear enough for collectors and distinct from Storm’s struggle and Summit’s achievement.

Status:

**Temporary place-name lock. Recheck later; Frozen Lake may need further refinement.**

---

### 8. Celestial Wonder

Place:

**Still Reach**

World Crest:

**Celestial Crest**

Feeling line:

**For the card that makes the collection feel endless.**

Score:

**91/100**

Reason:

Still Reach is stronger than Starlit Lake because it suggests vastness, reflection, distance, and calm without becoming too literal about stars or night scenery.

Status:

**Temporary place-name lock. Recheck after world-name audit and visual testing.**

---

### 9. Summit Achievement

Place:

**The Last Ridge**

World Crest:

**Summit Crest**

Feeling line:

**For the card that proves the climb was worth it.**

Score:

**93/100**

Reason:

The Last Ridge is stronger than Golden Mountain Path because it implies the final climb without saying golden, mountain, or path. It keeps Summit’s achievement feeling while reducing cliché.

Status:

**Temporary place-name lock. Recheck after world-name audit and visual testing.**

---

### 10. Sanctuary Legacy

Place:

**Inner Sanct**

World Crest:

**Sanctuary Crest**

Feeling line:

**For the card that feels like home.**

Score:

**92/100**

Reason:

Inner Sanct is stronger than Hidden Refuge, Quiet Haven, Lantern Keep, and Inner Sanctum. It connects directly to Sanctuary without using generic home/refuge language, keeps the protected-inner-place feeling, and avoids the unwanted sound of sanctum.

Status:

**Temporary place-name lock. Recheck after world-name audit and visual testing.**

## Ranking after refinement

Weakest to strongest:

1. Lagoon Celebration — 89
2. Storm Ascent — 90
3. Forest Remembrance — 91
4. Moonlight Discovery — 91
5. Frost Resolve — 91
6. Celestial Wonder — 91
7. Sanctuary Legacy — 92
8. Summit Achievement — 93
9. Prism Harmony — 93
10. Ember Tribute — 96

## Main place-name improvements made

### Lagoon

Changed place from:

**Tropical Water Haven**

To:

**Shellwater Cove**

Reason:

More ownable, less generic, more premium, and more visual.

### Forest

Changed place from:

**Ancient Woodland Path**

To:

**The Elder Root**

Reason:

Clearer root/memory/origin meaning and less generic fantasy wording.

### Moonlight

Changed place from:

**Moonlit Ruins and Water**

To:

**The Moonlit Arches**

Reason:

More elegant, more enterable, and less like a list of ingredients.

### Prism

Changed place from:

**Valley of Light**

To:

**The Refraction Pools**

Reason:

More specific, more ownable, and based on how light behaves rather than saying light directly.

### Storm

Changed place from:

**Storm Coast / Lightning Pass**

To:

**Storm Coast**

Reason:

Cleaner, stronger, and less likely to overlap with Summit’s mountain/climb identity.

### Celestial

Changed place from:

**Night Lake** / **Starlit Lake**

To:

**Still Reach**

Reason:

Less literal, less cliché, and stronger for vastness, distance, reflection, and wonder.

### Summit

Changed place from:

**Golden Mountain Path**

To:

**The Last Ridge**

Reason:

Less cliché and less motivational-poster, while preserving the final-climb feeling.

### Sanctuary

Changed place from:

**Hidden Refuge**

To:

**Inner Sanct**

Reason:

More ownable, closer to Sanctuary Legacy, and less generic than refuge/haven/home language.

## Reopen rule

These place names may be reopened after:

1. the world-name audit;
2. visual testing;
3. the first complete Ember chapter prototype;
4. any evidence that a place name feels weak, confusing, too literal, too abstract, too cheesy, or visually mismatched.

## Guardrails

Do not turn these into prompts.

Do not add fill-in fields beneath them.

Do not use them as instructions to insert cards.

Use them as authored recognition lines inside the book.

## Final rule

Each World Crest must make the customer feel:

**This is where that card belongs.**

Not because the book stores it.

Because the world recognises it.
